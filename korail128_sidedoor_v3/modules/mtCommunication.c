					...
					if((Slave_up_data == aUartRx1Buffer[9]) && (Slave_down_data == aUartRx1Buffer[10]))
					{
						if(aUartRx1Buffer[1] == g_unDeviceID) // 마스터에서 자신을 요청한 경우
						{
							...
							if(m_RtcDefine == SET_NONE)
							{
								...
								if((aUartRx1Buffer[4] == rs485_rtctime.ucYears) && (aUartRx1Buffer[5] == rs485_rtctime.ucMonth) && (aUartRx1Buffer[6] == rs485_rtctime.ucDate))
								{
									if((aUartRx1Buffer[7] == rs485_rtctime.ucHours) && (aUartRx1Buffer[8] == rs485_rtctime.ucMinutes))
									{
										...
										rtc_time.ucSeconds = aUartRx1Buffer[9];
								...
								{
									if(((((aUartRx1Buffer[4]>>4)*10)+(aUartRx1Buffer[4]&0x0f)) > 18) && ((((aUartRx1Buffer[4]>>4)*10)+(aUartRx1Buffer[4]&0x0f)) < 100))
									{
										rtc_time.ucYears = aUartRx1Buffer[4];
										rtc_time.ucMonth = aUartRx1Buffer[5];
										rtc_time.ucDate	 = aUartRx1Buffer[6];
									...
									else if((rtc_time2.ucYears > 18) && (rtc_time2.ucYears < 100))
									{
										/* 이 hex값이 rs485로 전송하게 된다.*/
										rtc_time.ucYears = rs485_rtctime.ucYears;
										rtc_time.ucMonth = rs485_rtctime.ucMonth;
										rtc_time.ucDate	 = rs485_rtctime.ucDate;
										rtc_time.ucHours = rs485_rtctime.ucHours;
										rtc_time.ucMinutes = rs485_rtctime.ucMinutes;
										rtc_time.ucSeconds = rs485_rtctime.ucSeconds;
...
*/
			...
			if(PortNumber==DCU_ALL_MVB_SINK_COMMON)
			{
				...
				{
					MatchCount++;
					if((MatchCount>6) && ((m_RtcDefine == SET_NONE)))
					{
#if 1 //jeon_200216

						rtc_time2 = rtc_get_time(); //RTC Read Time
						
						if((spiRxBuffer[2] == rtc_time2.ucYears) && (spiRxBuffer[3] == rtc_time2.ucMonth) && (spiRxBuffer[4] == rtc_time2.ucDate))
						{
							if((spiRxBuffer[5] == rtc_time2.ucHours) && (spiRxBuffer[6] == rtc_time2.ucMinutes))
							{
								/* 이 hex값이 rs485로 전송하게 된다.*/
						...
						{
							if(((((spiRxBuffer[2]>>4)*10)+(spiRxBuffer[2]&0x0f)) > 18) && ((((spiRxBuffer[2]>>4)*10)+(spiRxBuffer[2]&0x0f)) < 100))
							{
								/* 이 hex값이 rs485로 전송하게 된다.*/
								rtc_time.ucYears = spiRxBuffer[2];
								rtc_time.ucMonth = spiRxBuffer[3];
							...
							{
								/* 이 hex값이 rs485로 전송하게 된다.*/
								rtc_time.ucYears = rtc_time2.ucYears;
								rtc_time.ucMonth = rtc_time2.ucMonth;
								rtc_time.ucDate	 = rtc_time2.ucDate;
								rtc_time.ucHours = rtc_time2.ucHours;
								rtc_time.ucMinutes = rtc_time2.ucMinutes;
								rtc_time.ucSeconds = rtc_time2.ucSeconds;
...
//				#endif
	...
	//							debug("TCMS->MVB로부터 젤체 원복 비트 수신 \r\n");
				...
				else																		// TCMS가 패킷을 보내고 있지 않아서 같은 패킷이 계속 읽힘
				{
					...
					else if((get_diff_tick(osKernelSysTick(),error_list_time[DCU_MVB_ERROR]) > 70000) && (get_diff_tick(osKernelSysTick(),error_list_time[DCU_MVB_ERROR]) < 80000))
//					else if((get_diff_tick(osKernelSysTick(),error_list_time[DCU_MVB_ERROR]) > 20000) && (get_diff_tick(osKernelSysTick(),error_list_time[DCU_MVB_ERROR]) < 30000))
					...
					else if(get_diff_tick(osKernelSysTick(),error_list_time[DCU_MVB_ERROR]) > 60000)
