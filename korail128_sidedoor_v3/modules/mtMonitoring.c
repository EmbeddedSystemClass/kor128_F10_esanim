...
void mm_SaveFaultDcuData(uint8_t ErrorCode)
{
	...
	if(((fault_rtcdata.ucYears < 18) || (fault_rtcdata.ucYears > 100)) || \ 
	   ((fault_rtcdata.ucMonth < 1) || (fault_rtcdata.ucMonth > 12)) || \
	   ((fault_rtcdata.ucDate < 1) || (fault_rtcdata.ucDate > 31)))  //jeon_200217
	  {
		fault_rtcdata.ucYears = rtc_time.ucYears;
		fault_rtcdata.ucMonth = rtc_time.ucMonth;
		fault_rtcdata.ucDate = rtc_time.ucDate;
		fault_rtcdata.ucHours = rtc_time.ucHours;
		fault_rtcdata.ucMinutes = rtc_time.ucMinutes;
		fault_rtcdata.ucSeconds = rtc_time.ucSeconds;
	  }
#endif	
...
void mm_OpenFault_Detection(void)
{
	...
	if(mdc_DoorState == DOOR_OPENING)
	{
		if(get_diff_tick(osKernelSysTick(),error_list_time[DCU_OPEN_FAULT]) > 3000)
		{
